...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
  ...
  ml_dsa_87_keygen(pk, sk, randomness);
  return 0;
}
...
int crypto_sign_signature_ctx(uint8_t *sig,
  ...
  randombytes(randomness, 32);
  int result = ml_dsa_87_sign(sig, context_message_randomness, contextlen_messagelen, sk);
                       ...
                       const uint8_t *pk)
{
  ...
  (void)siglen;
  int result = ml_dsa_87_verify(sig, context_message, contextlen_messagelen, pk);
